...
//if($kd=="LC"){
     include "koneksi.php";
     $no = mysqli_query($koneksi,"SELECT no_form FROM form_list WHERE no_form like '%LR%' ORDER BY id DESC");
     ...
     if ($urut2!==$thn){
        $format = "LR-" .$bln."-".$thn."-"."0001";
     }else{
         if (strlen($tambah) == 1){
           $format = "LR-".$bln."-".$thn."-"."000".$tambah;
         }else if (strlen($tambah) == 2) {
           $format = "LR-".$bln."-".$thn."-"."00".$tambah;
         }else if (strlen($tambah) == 3) {
           $format = "LR-".$bln."-".$thn."-"."0".$tambah;
         }
         else{
           $format = "LR-".$bln."-".$thn."-".$tambah;
         }
     };
    //}
...
<form action="barang_tambah_form_list.php" method="post">
...
<div class="container modal-lg p-3 border" style="width:60%; margin-left:27%">
  <table class="table" border="1" width="100%">
                ...
                <tr>
                    ...
                    <td rowspan="2"><center><h3><b>LOGBOOK LIST DATA<br>RETAIN</b></h3></center></td>
  ...
  <input type="hidden" class="form-control" value="<?php echo $format?>" name="no_form" readonly required>
           <tr>
                    ...
                    <td><input type="text" class="form-control" name="kd_list" required></td>
            </tr>
        <tr>
                    <td class="bg-light" width="28%">Nama Konsumen</td>
                    <td><input type="text" class="form-control" name="nm_kons" required></td>
            </tr>
            <tr>
                    <td class="bg-light" width="28%">Nama Produk</td>
                    <td><input type="text" class="form-control" name="nm_prod" required></td>
            </tr>
            <tr>
                    <td class="bg-light" width="28%">Tanggal Fix</td>
                    <td> <input type="date" class="form-control" name="tgl_fix" required></td>
            </tr>
            <tr>
                    <td class="bg-light" width="28%">Jumlah</td>
                    <td> <input type="text" class="form-control" name="jml_list" required></td>
            </tr>
